...
if __name__ == '__main__':
	...
	model = CNNModel(n_tgt_vocab, dropout=dropout)
	# model = nn.DataParallel(model, output_device=device)
	model.to(device)

	if load_model and os.path.isfile('../saved_models/cnn_based/full_model.pt'):
		model.load_state_dict(torch.load('../saved_models/cnn_based/full_model.pt'))
